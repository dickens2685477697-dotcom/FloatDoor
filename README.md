...
**请先阅读 [《使用须知与责任限制》](USAGE_TERMS.md)，尤其是无担保、责任限制及法定权利保留条款。** Float Door 用于内容暂存与转移，不应作为重要数据的唯一存储或备份方案。请保留原始副本并独立备份，核对保存和传输结果；一次性内容会依保存策略被清理。“长期素材”表示不会按到期策略自动清理，不是永久保存保证。
